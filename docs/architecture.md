...
**SPA caching**: Partial SPA calculation results are cached when processing multiple positions for the same datetime, significantly improving performance for time series. The cache uses bounded eviction to keep memory usage stable even for long-running streams.
